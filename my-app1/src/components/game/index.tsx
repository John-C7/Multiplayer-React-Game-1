...
const GameContainer = styled.div`
  ...
  font-family: 'Orbitron', sans-serif; /* Changed to a sci-fi font */
  position: relative;
...
const WinnerConfetti = styled(Confetti)`
  position: absolute;
  top: 0;
  left: 0;
`;
...
export { GameContainer, RowContainer, Cell, PlayStopper, X, O };
...
export function Game() {
  ...
  const isFirstTurn = playerSymbol === "x"; 
  ...
  const updateGameMatrix = (column: number, row: number, symbol: "x" | "o") => {
    if (!isGameStarted || (!isPlayerTurn && isFirstTurn)) {
      return; 
    }
...
return (
  <GameContainer>
    ...
    {isFirstTurn && isPlayerTurn && <h2>You are playing as X</h2>}
    {!isFirstTurn && isPlayerTurn && <h2>You are playing as O</h2>}
    {!isGameStarted || !isPlayerTurn && <PlayStopper />}
    {!isPlayerTurn && triggerConfetti && (
      <WinnerConfetti
        numberOfPieces={250}
        recycle={true}
        width={600}
